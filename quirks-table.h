...
// SIMCom modules with USB Audio 
// SIMCom SIM7600 with PCM format from application note 

/* SIMTech SIM7600 with BULK audio endpoints */
{
	USB_DEVICE(0x1e0e, 0x9001),
	.driver_info = (unsigned long) &(const struct snd_usb_audio_quirk) {
		.vendor_name = "SimTech_ONIIP",
		.product_name = "SIM7600",
		.ifnum = 4,
		.type = QUIRK_COMPOSITE,
		.data = &(const struct snd_usb_audio_quirk[]) {
			{
				/* Playback (TX to modem) */
				.ifnum = 4,
				.type = QUIRK_AUDIO_FIXED_ENDPOINT,
				.data = &(const struct audioformat) {
					.formats = SNDRV_PCM_FMTBIT_S16_LE,
					...
					.attributes = 0,
					.endpoint = 0x05,
					.ep_attr = USB_ENDPOINT_XFER_BULK,
					.rates = SNDRV_PCM_RATE_8000,
					.rate_min = 8000,
					.rate_max = 8000,
					.nr_rates = 1,
					.rate_table = (unsigned int[]) { 8000 },
					.maxpacksize = 512,
				}
			},
			{
				/* Capture (RX from modem) */
				.ifnum = 4,
				.type = QUIRK_AUDIO_FIXED_ENDPOINT,
				.data = &(const struct audioformat) {
					.formats = SNDRV_PCM_FMTBIT_S16_LE,
...
/*
{
    USB_DEVICE(0x1e0e, 0x9001),
    .driver_info = (unsigned long) &(const struct snd_usb_audio_quirk) {
        .vendor_name = "SimTech, Incorporated",
        .product_name = "SIM7600 Voice Modem",
        .ifnum = 4,
        .type = QUIRK_AUDIO_FIXED_ENDPOINT,
        .data = &(const struct audioformat) {
            .formats = SNDRV_PCM_FMTBIT_S16_LE,
            ...
            .altset_idx = 1,
            .attributes = 0,
            .endpoint = 0x05,  // Playback (TX) - в модем
            .ep_attr = USB_ENDPOINT_XFER_BULK,
            .rates = SNDRV_PCM_RATE_8000 | SNDRV_PCM_RATE_16000,
            .rate_min = 8000,
            .rate_max = 16000,
            .nr_rates = 2,
            .rate_table = (unsigned int[]) { 8000, 16000 },
            .maxpacksize = 512,
        }
    }
},
{
    ...
    .driver_info = (unsigned long) &(const struct snd_usb_audio_quirk) {
        ...
        .data = &(const struct audioformat) {
            ...
            .altset_idx = 1,
            .attributes = 0,
            .endpoint = 0x88,  // Capture (RX) - из модема
            .ep_attr = USB_ENDPOINT_XFER_BULK,
            .rates = SNDRV_PCM_RATE_8000 | SNDRV_PCM_RATE_16000,
            .rate_min = 8000,
            .rate_max = 16000,
...
// SIMCom SIM7600 with PCM format from application note 
{
    ...
    .driver_info = (unsigned long) &(const struct snd_usb_audio_quirk) {
        ...
        .type = QUIRK_AUDIO_FIXED_ENDPOINT,
        .data = &(const struct audioformat) {
            .formats = SNDRV_PCM_FMTBIT_S16_LE,  // 16-bit linear
            .channels = 1,                       // Mono
            .fmt_type = UAC_FORMAT_TYPE_I,
            .rates = SNDRV_PCM_RATE_8000,        // 8K sample rate
            .rate_min = 8000,
            .rate_max = 16000,                    // или 16000 для SIM7600
        }
    }
},
*/
