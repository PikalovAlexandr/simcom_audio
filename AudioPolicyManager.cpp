...
static constexpr const char kSimcomForceStartInputProp[] =
                                                         ...
                                                         audio_format_t encodedFormat)
{
    ...
    if (audio_is_output_device(deviceType)) {
        ...
        case AUDIO_POLICY_DEVICE_STATE_AVAILABLE: {
            ...
            const bool skipOutputCheck =
                    (deviceType == AUDIO_DEVICE_OUT_TELEPHONY_TX);
            if (skipOutputCheck) {
                ALOGI("%s() skipping checkOutputsForDevice for Telephony Tx", __func__);
            } else {
        ...
        case AUDIO_POLICY_DEVICE_STATE_UNAVAILABLE: {
            ...
            if (skipOutputCheck) {
                ALOGI("%s() skipping checkOutputsForDevice on disconnect for Telephony Tx", __func__);
            } else {
...
uint32_t AudioPolicyManager::updateCallRouting(const DeviceVector &rxDevices, uint32_t delayMs)
{
    ...
    if (mCallRoutingInProgress) {
        ALOGW("SIMCOM updateCallRouting: call already in progress, ignoring duplicate request");
        return muteWaitMs;
    }
    struct CallRoutingAutoReset {
        bool *flag;
        ~CallRoutingAutoReset() {
            if (flag != nullptr) {
                *flag = false;
            }
        }
    } autoReset{&mCallRoutingInProgress};
    mCallRoutingInProgress = true;
    ...
    ALOGI("SIMCOM updateCallRouting: telephony modules rx=%p (primary=%d) tx=%p (primary=%d)",
          telephonyRxModule.get(), telephonyRxModule != nullptr ?
                  isPrimaryModule(telephonyRxModule) : -1,
          telephonyTxModule.get(), telephonyTxModule != nullptr ?
                  isPrimaryModule(telephonyTxModule) : -1);
    ...
    ALOGI("SIMCOM updateCallRouting: rxSource=%s txSink=%s",
          rxSourceDevice != nullptr ? rxSourceDevice->toString().c_str() : "<null>",
          txSinkDevice != nullptr ? txSinkDevice->toString().c_str() : "<null>");

    if (rxSourceDevice == nullptr && telephonyRxModule != nullptr &&
            isPrimaryModule(telephonyRxModule)) {
        sp<DeviceDescriptor> declaredRx =
                telephonyRxModule->getDeclaredDevices().getDevice(
                        AUDIO_DEVICE_IN_TELEPHONY_RX, String8(), AUDIO_FORMAT_DEFAULT);
        if (declaredRx != nullptr) {
            const char *moduleName = telephonyRxModule->getName();
            if (moduleName == nullptr || moduleName[0] == '\0') {
                moduleName = "unknown";
            }
            ALOGI("SIMCOM updateCallRouting: auto-connecting Telephony Rx, module=%s addr=%s",
                  moduleName, declaredRx->address().string());
            ALOGI("SIMCOM updateCallRouting: setDeviceConnectionStateInt(Rx) addr=%s tag=%s",
                  declaredRx->address().string(), declaredRx->getTagName().string());
            status_t status = setDeviceConnectionStateInt(
                    AUDIO_DEVICE_IN_TELEPHONY_RX,
                    AUDIO_POLICY_DEVICE_STATE_AVAILABLE,
                    declaredRx->address().string(),
                    declaredRx->getTagName().string(),
                    declaredRx->hasCurrentEncodedFormat() ?
                        declaredRx->getEncodedFormat() : AUDIO_FORMAT_DEFAULT);
            if (status == NO_ERROR || status == INVALID_OPERATION) {
                if (status == INVALID_OPERATION) {
                    ALOGW("SIMCOM updateCallRouting: Telephony Rx already registered (status=%d)",
                          status);
                } else {
                    ALOGI("SIMCOM updateCallRouting: Telephony Rx registered");
                }
                rxSourceDevice =
                        mAvailableInputDevices.getDevice(AUDIO_DEVICE_IN_TELEPHONY_RX,
                                                         String8(),
                ...
                ALOGW("updateCallRouting() failed to connect Telephony Rx: %d", status);
            }
        } else {
            const char *moduleName = telephonyRxModule->getName();
            if (moduleName == nullptr || moduleName[0] == '\0') {
            ...
            ALOGW("updateCallRouting() Telephony Rx declared device missing on module %s",
                  moduleName);
        }
    }

    if (txSinkDevice == nullptr && telephonyTxModule != nullptr &&
            isPrimaryModule(telephonyTxModule)) {
        sp<DeviceDescriptor> declaredTx =
                telephonyTxModule->getDeclaredDevices().getDevice(
                        AUDIO_DEVICE_OUT_TELEPHONY_TX, String8(), AUDIO_FORMAT_DEFAULT);
        if (declaredTx != nullptr) {
            const char *moduleName = telephonyTxModule->getName();
            if (moduleName == nullptr || moduleName[0] == '\0') {
                moduleName = "unknown";
            }
            ALOGI("SIMCOM updateCallRouting: auto-connecting Telephony Tx, module=%s addr=%s",
                  moduleName, declaredTx->address().string());
            audio_format_t encoded =
                    declaredTx->hasCurrentEncodedFormat() ?
                    declaredTx->getEncodedFormat() : AUDIO_FORMAT_DEFAULT;
            ALOGI("SIMCOM updateCallRouting: setDeviceConnectionStateInt(Tx) addr=%s tag=%s",
                  declaredTx->address().string(), declaredTx->getTagName().string());
            status_t status = setDeviceConnectionStateInt(
                    AUDIO_DEVICE_OUT_TELEPHONY_TX,
                    AUDIO_POLICY_DEVICE_STATE_AVAILABLE,
                    declaredTx->address().string(),
                    declaredTx->getTagName().string(),
                    encoded);
            if (status == NO_ERROR || status == INVALID_OPERATION) {
                if (status == INVALID_OPERATION) {
                    ALOGW("SIMCOM updateCallRouting: Telephony Tx already registered (status=%d)",
                          status);
                } else {
                    ALOGI("SIMCOM updateCallRouting: Telephony Tx registered");
                }
                txSinkDevice =
                        mAvailableOutputDevices.getDevice(AUDIO_DEVICE_OUT_TELEPHONY_TX,
                                                          String8(),
                ...
                ALOGW("updateCallRouting() failed to connect Telephony Tx: %d", status);
            }
        } else {
            const char *moduleName = telephonyTxModule->getName();
            if (moduleName == nullptr || moduleName[0] == '\0') {
            ...
            ALOGW("updateCallRouting() Telephony Tx declared device missing on module %s",
                  moduleName);
        }
    }

    ALOGI("SIMCOM updateCallRouting: post-connect txSink=%s",
          txSinkDevice != nullptr ? txSinkDevice->toString().c_str() : "<null>");

    bool baseRxPatch = false;
    bool baseTxPatch = false;
    if (isPrimaryModule(telephonyRxModule) && isPrimaryModule(telephonyTxModule) &&
        ...
        if (rxSourceDevice == 0 || txSinkDevice == 0) {
            ALOGE("SIMCOM updateCallRouting: missing devices rx=%s tx=%s (HALv%u)",
                  rxSourceDevice != nullptr ? rxSourceDevice->toString().c_str() : "<null>",
                  txSinkDevice != nullptr ? txSinkDevice->toString().c_str() : "<null>",
                  telephonyRxModule != nullptr ? telephonyRxModule->getHalVersionMajor() : 0);
            ALOGE("updateCallRouting() no telephony Tx and/or RX device");
            return muteWaitMs;
        }
        const auto &primaryModule = telephonyRxModule;
        baseRxPatch = !primaryModule->supportsPatch(rxSourceDevice, rxSinkDevice);
        baseTxPatch = !primaryModule->supportsPatch(txSourceDevice, txSinkDevice);
    } else {
        baseRxPatch = !(availablePrimaryOutputDevices().contains(rxSinkDevice)) &&
                (rxSourceDevice != 0);
        baseTxPatch = !(availablePrimaryModuleInputDevices().contains(txSourceDevice)) &&
                (txSinkDevice != 0);
    }

    const bool telephonyDevicesDetected =
            rxSourceDevice != nullptr &&
            rxSourceDevice->type() == AUDIO_DEVICE_IN_TELEPHONY_RX &&
            txSinkDevice != nullptr &&
    ...
    if (telephonyDevicesDetected && inCallMode) {
        if (!createRxPatch) {
            ALOGI("SIMCOM: Telephony devices detected in call mode, forcing RX patch");
            createRxPatch = true;
        }
        if (!createTxPatch) {
            ALOGI("SIMCOM: Telephony devices detected in call mode, forcing TX patch");
            createTxPatch = true;
        }
    }
    ...
    if (createRxPatch) {
        if (isTelephonyPatchActive(true, rxSinkDevice)) {
            ALOGI("SIMCOM: reusing existing RX patch for %s",
                  rxSinkDevice->toString().c_str());
            needRxPatch = false;
        } else if (mCallRxPatch != 0) {
            releaseTelephonyPatch(true, "RX device changed");
        }
    } else {
        releaseTelephonyPatch(true, "RX patch disabled");
    }
    if (createTxPatch) {
        if (isTelephonyPatchActive(false, txSourceDevice)) {
            ALOGI("SIMCOM: reusing existing TX patch for %s",
                  txSourceDevice != nullptr ? txSourceDevice->toString().c_str() : "<null>");
            needTxPatch = false;
        } else if (mCallTxPatch != 0) {
            releaseTelephonyPatch(false, "TX device changed");
        }
    } else {
        releaseTelephonyPatch(false, "TX patch disabled");
    }
    ...
    if (!createRxPatch) {
        ...
        if (rxPatch != 0 && rxSinkDevice != nullptr) {
            mCallRxPatch = rxPatch;
            mCallRxDeviceId = rxSinkDevice->getId();
        } else {
    ...
    ALOGI("SIMCOM updateCallRouting: createRxPatch=%d createTxPatch=%d muteWait=%u",
          createRxPatch, createTxPatch, muteWaitMs);

    if (createTxPatch && needTxPatch) { // create TX path audio patch
        sp<AudioPatch> txPatch = createTelephonyPatch(false /*isRx*/, txSourceDevice, delayMs);
        ...
        ALOGI("SIMCOM updateCallRouting: TX patch not required (sink on primary=%d avail=%s)",
              availablePrimaryModuleInputDevices().contains(txSourceDevice),
              txSinkDevice != nullptr ? txSinkDevice->getTagName().string() : "<null>");
    }

    dumpTelephonyPatches();
    return muteWaitMs;
}
...
void AudioPolicyManager::simcomReleaseVoiceCallInput() {
    if (mSimcomVoicePortId == AUDIO_PORT_HANDLE_NONE) {
        return;
    }
    ALOGI("SIMCOM ensureVoiceCallInput: stopping voice_call_input portId=%d io=%d",
          mSimcomVoicePortId, mSimcomVoiceInputHandle);
    stopInput(mSimcomVoicePortId);
    releaseInput(mSimcomVoicePortId);
...
void AudioPolicyManager::simcomHandleVoiceCallInput(bool enable) {
    bool forceEnabled = isSimcomForceStartInputEnabled();
    ALOGE("SIMCOM handleVoiceCallInput(entry): enable=%d forceEnabled=%d activePort=%d "
          "activeIo=%d phoneState=%d inputType=%d",
          enable, forceEnabled, mSimcomVoicePortId, mSimcomVoiceInputHandle,
          mEngine->getPhoneState(), mSimcomVoiceInputType);

    if (!forceEnabled) {
        if (!enable && mSimcomVoicePortId != AUDIO_PORT_HANDLE_NONE) {
            simcomReleaseVoiceCallInput();
        } else if (enable && mSimcomVoicePortId != AUDIO_PORT_HANDLE_NONE) {
            // Property disabled while input active; release to avoid leaks.
            simcomReleaseVoiceCallInput();
        }
        return;
    }
    ...
    if (enable) {
        ...
        status_t status = getInputForAttr(&attr, &input, AUDIO_UNIQUE_ID_ALLOCATE,
                                          ...
                                          &inputType, &portId);
        if (status != NO_ERROR) {
            ALOGE("SIMCOM ensureVoiceCallInput: getInputForAttr failed status=%d", status);
            return;
        }

        status = startInput(portId);
        if (status != NO_ERROR) {
            ALOGE("SIMCOM ensureVoiceCallInput: startInput failed portId=%d status=%d",
                  portId, status);
            releaseInput(portId);
            mSimcomVoicePortId = AUDIO_PORT_HANDLE_NONE;
            mSimcomVoiceInputHandle = AUDIO_IO_HANDLE_NONE;
        ...
        mSimcomVoicePortId = portId;
        mSimcomVoiceInputHandle = input;
        mSimcomVoiceDeviceId = deviceId;
        mSimcomVoiceInputType = inputType;
        ALOGE("SIMCOM ensureVoiceCallInput: started voice_call_input io=%d portId=%d devId=%d",
              mSimcomVoiceInputHandle, mSimcomVoicePortId, mSimcomVoiceDeviceId);
        return;
    }

    if (mSimcomVoicePortId != AUDIO_PORT_HANDLE_NONE) {
        ALOGE("SIMCOM ensureVoiceCallInput: stopping voice_call_input portId=%d io=%d",
              mSimcomVoicePortId, mSimcomVoiceInputHandle);
    }
    simcomReleaseVoiceCallInput();
}
sp<AudioPatch> AudioPolicyManager::createTelephonyPatch(
    ...
    ALOGI("SIMCOM: createTelephonyPatch START: isRx=%d device=%s delayMs=%u",
          isRx, device != nullptr ? device->toString().c_str() : "null", delayMs);
    PatchBuilder patchBuilder;

    if (device == nullptr) {
        ALOGE("SIMCOM: createTelephonyPatch FAILED - null device");
        return nullptr;
    }
    if (isRx) {
        ...
        patchBuilder.addSink(mAvailableOutputDevices.getDevice(
                    AUDIO_DEVICE_OUT_TELEPHONY_TX, String8(), AUDIO_FORMAT_DEFAULT));
    }
    ...
    if (isRx) {
        ...
        // For TX path: get output stream for Telephony TX device
        // This ensures the output stream is opened with AUDIO_DEVICE_OUT_TELEPHONY_TX,
        // which allows HAL to properly configure is_simcom_voice and resampler
        DeviceVector txDevices;
        txDevices.add(mAvailableOutputDevices.getDevice(
                    AUDIO_DEVICE_OUT_TELEPHONY_TX, String8(), AUDIO_FORMAT_DEFAULT));
        if (txDevices.isEmpty()) {
            ALOGW("SIMCOM: createTelephonyPatch() TX path: Telephony TX device not available");
        } else {
            SortedVector<audio_io_handle_t> outputs =
                    getOutputsForDevices(txDevices, mOutputs);
            if (!outputs.isEmpty()) {
                output = selectOutput(outputs);
    ...
    ALOGI("SIMCOM: createTelephonyPatch() outputDevice=%s selected output=%d (isRx=%d)",
          outputDevice != nullptr ? outputDevice->toString().c_str() : "null", output, isRx);
    ...
    if (output != AUDIO_IO_HANDLE_NONE) {
        ...
        if (outputDesc != nullptr) {
            ALOG_ASSERT(!outputDesc->isDuplicated(), "%s() %s device output %d is duplicated", __func__,
                        outputDevice != nullptr ? outputDevice->toString().c_str() : "null", output);
            ALOGI("SIMCOM: createTelephonyPatch() adding output stream %d to patch (isRx=%d)", output, isRx);
            patchBuilder.addSource(outputDesc, { .stream = AUDIO_STREAM_PATCH });
        } else {
            ALOGW("SIMCOM: createTelephonyPatch() outputDesc is null for output %d (isRx=%d)", output, isRx);
        }
    } else {
    ...
    ALOGI("SIMCOM: createTelephonyPatch() calling createAudioPatch: isRx=%d patch num_sources=%u num_sinks=%u",
          isRx, patch->num_sources, patch->num_sinks);
    status_t status = mpClientInterface->createAudioPatch(
            patch, &afPatchHandle, delayMs);
    ALOGW_IF(status != NO_ERROR,
            "SIMCOM: %s() error %d creating %s audio patch (num_sources=%u num_sinks=%u)", __func__, status, isRx ? "RX" : "TX",
            patch->num_sources, patch->num_sinks);
    ALOGI("SIMCOM: createTelephonyPatch RESULT: status=%d handle=%d (isRx=%d)",
          status, afPatchHandle, isRx);
...
void AudioPolicyManager::dumpTelephonyPatches() const {
    ALOGI("SIMCOM: Current telephony patches - Rx:%s (devId=%d) Tx:%s (devId=%d)",
          mCallRxPatch != nullptr ? "EXISTS" : "NULL",
          mCallRxDeviceId,
          mCallTxPatch != nullptr ? "EXISTS" : "NULL",
          mCallTxDeviceId);
    if (mCallRxPatch != nullptr) {
        ALOGI("SIMCOM: RxPatch handle=%d", mCallRxPatch->mAfPatchHandle);
    }
    if (mCallTxPatch != nullptr) {
        ALOGI("SIMCOM: TxPatch handle=%d", mCallTxPatch->mAfPatchHandle);
    }
}

bool AudioPolicyManager::isTelephonyPatchActive(bool isRx,
        const sp<DeviceDescriptor>& device) const {
    if (device == nullptr) {
        return false;
    }
    const sp<AudioPatch> &patch = isRx ? mCallRxPatch : mCallTxPatch;
    audio_port_handle_t trackedId =
            isRx ? mCallRxDeviceId : mCallTxDeviceId;
    return (patch != 0) && (trackedId == device->getId());
}

void AudioPolicyManager::releaseTelephonyPatch(bool isRx, const char *reason) {
    sp<AudioPatch> &patch = isRx ? mCallRxPatch : mCallTxPatch;
    audio_port_handle_t &trackedId =
            isRx ? mCallRxDeviceId : mCallTxDeviceId;
    if (patch != 0) {
        ALOGI("SIMCOM: releasing %s patch handle=%d reason=%s",
              isRx ? "RX" : "TX",
              patch->mAfPatchHandle,
              reason != nullptr ? reason : "unknown");
        mpClientInterface->releaseAudioPatch(patch->mAfPatchHandle, 0);
        patch.clear();
    }
    trackedId = AUDIO_PORT_HANDLE_NONE;
}
...
void AudioPolicyManager::setPhoneState(audio_mode_t state)
{
    ...
    const bool simcomVoiceActive =
            (state == AUDIO_MODE_IN_CALL) || (state == AUDIO_MODE_IN_COMMUNICATION);
    if (simcomVoiceActive) {
        // Ensure voice call input is started before creating telephony patches so that
        // the patch builder can find a matching input stream.
        simcomHandleVoiceCallInput(true);
    }
...
status_t AudioPolicyManager::startInput(audio_port_handle_t portId)
{
    ...
    if (inputDesc == 0) {
        ALOGW("SIMCOM startInput: no input descriptor for portId=%d", portId);
        return BAD_VALUE;
    }
    ...
    const audio_attributes_t& attributes = client->attributes();
    const std::string attrStr = toString(attributes);
    ALOGI("SIMCOM startInput: portId=%d ioHandle=%d session=%d source=%d activeInputs=%zu "
          "client=%s attr=%s",
          portId, input, session, attributes.source, activeInputs.size(),
          client->toShortString().c_str(), attrStr.c_str());

    status_t status = inputDesc->start();
    if (status != NO_ERROR) {
        ALOGW("SIMCOM startInput: inputDesc->start() failed ioHandle=%d status=%d",
              input, status);
        return status;
    }
  ...
  // increment activity count before calling getNewInputDevice() below as only active sessions
    ...
    const std::string deviceStr =
            (device != nullptr) ? device->toString() : std::string("none");
    ALOGI("SIMCOM startInput: ioHandle=%d assigned device=%s activeCount=%u",
          input, deviceStr.c_str(), inputDesc->activeCount());
    ...
    ALOGI("SIMCOM startInput: finished ioHandle=%d source=%d activeCount=%u",
          input, client->source(), inputDesc->activeCount());
...
status_t AudioPolicyManager::stopInput(audio_port_handle_t portId)
{
    ...
    const audio_attributes_t& attributes = client->attributes();
    const std::string attrStr = toString(attributes);
    const uint32_t beforeCount = inputDesc->activeCount();
    ALOGI("SIMCOM stopInput: begin portId=%d ioHandle=%d session=%d source=%d activeCount=%u "
          "client=%s attr=%s",
          portId, input, client->session(), attributes.source, beforeCount,
          client->toShortString().c_str(), attrStr.c_str());
    ...
    if (inputDesc->isActive()) {
        sp<DeviceDescriptor> newDevice = getNewInputDevice(inputDesc);
        const std::string deviceStr =
                (newDevice != nullptr) ? newDevice->toString() : std::string("none");
        setInputDevice(input, newDevice, false /* force */);
        ALOGI("SIMCOM stopInput: input still active ioHandle=%d reassigned device=%s "
              "activeCount=%u",
              input, deviceStr.c_str(), inputDesc->activeCount());
    } else {
        sp<AudioPolicyMix> policyMix = inputDesc->mPolicyMix.promote();
    ...
    ALOGI("SIMCOM stopInput: finished portId=%d ioHandle=%d activeCount=%u",
          portId, input, inputDesc->activeCount());
    return NO_ERROR;
}
