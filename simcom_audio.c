...
static int simcom_audio_create_playback_urbs(struct simcom_audio *chip)
{
    ...
    for (i = 0; i < NUM_URBS; i++) {
        chip->urb_out[i] = usb_alloc_urb(0, GFP_KERNEL);
        if (!chip->urb_out[i])
            return -ENOMEM;
            
        usb_fill_bulk_urb(chip->urb_out[i], chip->udev,
...
static int simcom_playback_prepare(struct snd_pcm_substream *substream)
{
    ...
    pr_info("simcom_audio: playback prepare (8kHz, mono)\n");
    return 0;
}
    ...
    case SNDRV_PCM_TRIGGER_START:
        ...
        for (i = 0; i < NUM_URBS; i++) {
            if (chip->urb_out[i]) {
                ...
                if (ret < 0) {
                    pr_err("simcom_audio: failed to submit playback URB: %d\n", ret);
                    break;
                }
            }
        }
