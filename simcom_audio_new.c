...
static int simcom_pcm_open(struct snd_pcm_substream *substream)
{
    ...
    if (!chip) {
        pr_err("simcom_audio: chip is NULL in open\n");
        return -ENODEV;
    }

    runtime->hw = simcom_pcm_hw;
    return 0;
}
...
static int simcom_pcm_prepare(struct snd_pcm_substream *substream)
{
    ...
    if (!chip) {
        pr_err("simcom_audio: chip is NULL in prepare\n");
        return -ENODEV;
    }

    pr_info("simcom_audio: prepare stream %d\n", substream->stream);
    chip->running = 0;
    return 0;
}

/* Simple blocking I/O - нужно переписать на URBs! */
...
{
    /* TODO: реализовать правильный указатель позиции */
    return 0;
}
...
/* ========================================================= */
/* USB DRIVER CORE - ИСПРАВЛЕННАЯ ВЕРСИЯ                    */
/* ========================================================= */
                  ...
                  const struct usb_device_id *id)
{
    ...
    /* ВАЖНО: установить приватные данные для PCM */
